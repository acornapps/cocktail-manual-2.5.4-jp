...
| 処理 | Prometheusのログとkube-controller-managerの ログとイベントを確認する。 必要な場合には、 Podを再起動する。 |
...
| 処理 | Prometheusのログとkube-schedulerの ログとイベントを確認する。 必要な場合には、 Podを再起動する。 |
...
| 持続時間 | 15分 |
| 発生条件 | DaemonSetに状態が Readyではない Podがある場合に発生し |
...
| 持続時間 | 10分 |
| 発生条件 | DaemonSetに実行されてすることがPod数よりも、 実行中のPod数が小さい場合に発生し |
| 処理 | そのDaemonsetとPodのログとイベントを確認する。 配布がないされたノードが正常であることを確認する。 マスターノードが隔離された場合には、 Daemonsetにtoleration設定がされているかどうかを確認する。 |
...
| 持続時間 | 1時間 |
| 発生条件 | Nodeの状態が Readyではない場合に発生し |
| 処理 | そのノードの状態とイベントを確認する。 ssh経由でノードに接続してkubeletの状態を確認する。 |
...
| 持続時間 | 1分 |
| 発生条件 | クラスタ全体でNodeの 状態がReadyではない割合が 20％以上である場合に発生し |
| 処理 | そのノードの状態とイベントを確認する。 ssh経由でノードに接続してkubeletの状態を確認する。 |
...
| 処理 | Prometheusの ログとNodeExporterの ログとイベントを確認する。 必要な場合には、 Podを再起動する。 |
...
| 持続時間 | 30分 |
| 発生条件 | PVが要求されたディスクのサイズに比べ使用量が 80％を超えると発生し |
...
| 持続時間 | 30分 |
| 発生条件 | PVがマウントされたディスクのサイズに比べ使用量が 80％を超えると発生し |
...
| 持続時間 | 30分 |
| 発生条件 | Resource Limit設定値に比べCPU使用率が 90％を超えると発生し |
...
| 持続時間 | 30分 |
| 発生条件 | Resource Limit設定値に比べMemory使用率が 90％を超えると発生し |
