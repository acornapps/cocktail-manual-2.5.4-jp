...
또한 CUBE도구는 클러스터의 쿠버네티스 버전 업그레이드, 노드\(물리/가상 머신\)의 추가/삭제, 백업등의 클러스터 관리를 위한 기능을 제공한다.
...
* Monitoring : 인프라, 컨테이너, 서비스\(워크로드\)의 상태, 구성, 자원 모니터링
* Alerting : 특정 조건이 되었을 때 이메일, 메신져를 통한 경고 전파 및 관리
* Metering : 퍼블릭 클라우드 클러스터인 경우 사용 비용 조회
* Inspecting : 컨테이너 및 쿠버네스트 오브젝트의 변경 추적 및 이벤트 관리
